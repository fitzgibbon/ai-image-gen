* ai-image-gen

=ai-image-gen= generates images from Emacs through a small registry of
providers, and makes that available as an Org Babel language whose blocks
are prompts and whose results are image links.

Two provider types are built in:

- =ai-image-gen-openai= — the OpenAI images API (=/v1/images/generations=),
  or any server speaking it.
- =ai-image-gen-mlx= — [[https://github.com/cubist38/mlx-openai-server][mlx-openai-server]]'s variant, which also accepts
  =seed=, =steps= and =guidance_scale= and reports the seed of each image.

Requests go through [[https://github.com/alphapapa/plz.el][plz]], so they use curl and honour =CURL_CA_BUNDLE= for
self-signed local endpoints.

** Setup

#+begin_src emacs-lisp
(use-package ai-image-gen
  :vc (:url "https://github.com/fitzgibbon/ai-image-gen.git")
  :config
  (ai-image-gen-register-provider
   (ai-image-gen-mlx-create
    :name "flux2-klein"
    :url "https://localhost:11490/v1"
    :model "black-forest-labs/FLUX.2-klein-9B"))
  (ai-image-gen-register-provider
   (ai-image-gen-openai-create
    :name "gpt-image"
    :key #'my/get-openai-api-key
    :model "gpt-image-1"))
  (setq ai-image-gen-default-provider "flux2-klein")
  (add-to-list 'org-babel-load-languages '(ai-image-gen . t))
  (org-babel-do-load-languages 'org-babel-load-languages org-babel-load-languages))
#+end_src

=:key= may be a string or a function returning one; it is sent as a bearer
token. When exactly one provider is registered it is the default.

Images without an explicit destination go to =ai-image-gen-directory=.

Enabling the =ai-image-gen= Babel language (the last two lines above) makes
Org load the Babel support, which lives in =ob-ai-image-gen.el= because Org
finds a language's support by that name.

** Org Babel

The block body is the prompt:

#+begin_example
,#+begin_src ai-image-gen :size 768x512 :seed 7
A platypus in a yellow hard hat giving a thumbs up, flat vector illustration
,#+end_src

,#+RESULTS:
[[file:~/.emacs.d/ai-image-gen/20260923-234814-a-platypus-in-a-yellow-s7.png]]
#+end_example

Header arguments:

| argument    | meaning                                                 |
|-------------+---------------------------------------------------------|
| =:provider= | registered provider name (default provider if unset)    |
| =:size=     | =WIDTHxHEIGHT=                                          |
| =:seed=     | seed (=ai-image-gen-mlx= only)                          |
| =:steps=    | inference steps (=ai-image-gen-mlx= only)               |
| =:guidance= | guidance scale (=ai-image-gen-mlx= only)                |
| =:image=    | source image(s) to edit instead of generating           |
| =:mask=     | PNG marking what to change (=ai-image-gen-openai= only) |
| =:async=    | =yes= or =no=, overriding =ob-ai-image-gen-async=       |
| =:file=     | where to save the image                                 |

*** Background generation

Run with =C-c C-c=, a block returns at once: Org writes the result link
straight away, and the image fills it in (with its inline preview) when it
arrives, so you can keep working meanwhile. If generation fails, the link is
replaced by the error.

An edit whose =:image= source is still generating waits for it, then starts,
so a chain of blocks can be run in one go. Blocks run during export always
wait, since the exported document needs the image; so do blocks with
=:async no=, and all blocks when =ob-ai-image-gen-async= is nil.

*** Editing

With =:image=, a block edits a source image instead of generating from
nothing. A source is a file name, the name of another block (whose result
is used) or the name of a link:

#+begin_example
,#+name: mascot
,#+begin_src ai-image-gen :seed 21 :size 768x512
A cheerful platypus in a yellow hard hat, flat vector illustration
,#+end_src

,#+begin_src ai-image-gen :image mascot :seed 21
The same platypus, now riding a small red bicycle
,#+end_src
#+end_example

The second block edits whatever the first one last produced; a named block
with no result yet is run first. Re-running the first block changes what the
second edits.

Several sources are space-separated (=:image mascot ~/logo.png=) or a quoted
list (=:image '(mascot "~/a b.png")=). =:mask= takes the same kinds of source.

** Commands

=M-x ai-image-gen-insert= prompts for a description (the active region is the
default), generates asynchronously and inserts a reference at point: an Org
link with an inline preview in Org buffers, an inline MML image part in
message buffers, the file name elsewhere. With a prefix argument it asks
which provider to use.

=M-x ai-image-gen-edit= edits the image linked at point (or one read from the
minibuffer) as a prompt describes, and puts a link to the result on the line
after the original. With a prefix argument it replaces the original link.

** Lisp API

#+begin_src emacs-lisp
(let ((request (ai-image-gen-request-create
                :prompt "a lighthouse at dusk" :size "512x512" :n 2)))
  (ai-image-gen-generate
   request
   :provider "flux2-klein"
   :then (lambda (images)
           (dolist (image images)
             (ai-image-gen-image-save image (ai-image-gen-default-file request image))))
   :else (lambda (message) (message "failed: %s" message))))
#+end_src

=ai-image-gen-generate-sync= returns the list of =ai-image-gen-image= objects
instead, signalling =ai-image-gen-error= on failure. Each image carries its
bytes, sniffed MIME type and, where the provider reports one, its seed.

An edit is a different request type passed to the same functions; the
OpenAI-compatible providers send it to =/v1/images/edits=:

#+begin_src emacs-lisp
(ai-image-gen-generate-sync
 (ai-image-gen-edit-request-create
  :prompt "the same platypus, now holding a wrench"
  :images "~/Pictures/mascot.png"
  :seed 21))
#+end_src

=ai-image-gen-edit-request-create= checks that every source (and the mask)
is readable. =ai-image-gen-mlx= rejects a mask and =:n= above 1, since
mlx-openai-server's edits take neither.

** Adding a provider

A provider is a struct that includes =ai-image-gen-provider= and implements
=ai-image-gen-provider-generate=:

#+begin_src emacs-lisp
(cl-defstruct (my-provider (:include ai-image-gen-provider)
                           (:constructor my-provider-create))
  endpoint)

(cl-defmethod ai-image-gen-provider-generate ((provider my-provider) request then else)
  ;; Call THEN with a list of `ai-image-gen-image', or ELSE with a message.
  ;; Return the request process (or nil) so callers can cancel it.
  )
#+end_src

To support edits, add a method that also specialises the request on
=ai-image-gen-edit-request=.

Providers speaking a variant of the OpenAI API can instead include
=ai-image-gen-openai= and specialise =ai-image-gen-openai--body=, as
=ai-image-gen-mlx= does.

** Tests

#+begin_src sh
emacs --batch -L . -L tests -l ai-image-gen-tests -f ert-run-tests-batch-and-exit
#+end_src

=plz= must be on the load path. The live test is skipped unless
=AI_IMAGE_GEN_LIVE_URL= names an mlx-openai-server base URL (and optionally
=AI_IMAGE_GEN_LIVE_MODEL= its image model):

#+begin_src sh
AI_IMAGE_GEN_LIVE_URL=https://localhost:11490/v1 \
  emacs --batch -L . -L tests -l ai-image-gen-tests -f ert-run-tests-batch-and-exit
#+end_src
